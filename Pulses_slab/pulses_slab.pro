...
; --- and diamond12 -------------------------------------------
; --- to run program in idl: '.run pulses_slab' ----------------
...
; -------------------------------------------------------------
; -- 12.4.2012: use interpol_idl(..) instead of interpol(..) --
; -- to run with gdl  (interpol_idl is original idl routine) --
; -------------------------------------------------------------
;
...
pulse_sep=0.22       	; separation between pulses in microsec.
...
T_points=18000          ; points for spline interpotations
...
for i=1,pulse_number-1 do begin 
  ...
  while (j lt maxit) and time lt pulse_sep do begin 
    lambda_pulse=interpol(lambda,T_axis,T_pulse)
    cp_pulse=interpol(cp,T_axis,T_pulse)
    ;
...
print, 'final temperature after pulse train(calc):', T_final_pulse

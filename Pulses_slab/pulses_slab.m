...
function pulse_slab()
more off                % switch of output buffer (or use fflush for Octave)
...
T_points=18000;         % points for spline interpolations 50000
...
gauss_beam=1.0/(2*pi*sigma^2)*exp(max(-50, -(x_axis+dx/2).^2./(2*sigma^2)))*pulse_energy.*x_area;
...
for i=1:pulse_number-1 
  ...
  fprintf(1,'pulse number: %g \n', i) 
  fprintf(1,'  step, time step [10-8], total time [10-6s], T_max,  Q[pulses], Q_peak/Q_total \n')
  time=0.0;
  j=1; %  j=int32(1);
  % --- start individual pulse -----------------------------------
  while ((j< maxit)&(time< pulse_sep)) 
    lambda_pulse=interp1(T_axis, lambda,T_pulse,'*nearest');
    cp_pulse=interp1(T_axis, cp,T_pulse,'*nearest');
    %
    % --- calculate heat flows left and right of each element
    T_pulse_m1=circshift(T_pulse,[0,1]);
    j_left=-(T_pulse-T_pulse_m1).*lambda_pulse/dx;
    j_left=j_left*2*pi.*x_axis.*x_thick;
    j_right=circshift(j_left,[0,-1]);
    j_left(1)=0;
    j_right(x_points)=0;
    dT_dt=(j_left-j_right)./(cp_pulse*rho.*x_volume) ;
